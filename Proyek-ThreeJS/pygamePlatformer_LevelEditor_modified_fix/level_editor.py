...
platform = pygame.transform.scale(pygame.image.load('assets/images/tiles/rock.png'), (TILE_SIZE*5, TILE_SIZE))
...
def draw_inventory():
    ...
    level_text = font.render(f'Level: {active_level}', True, 'white')
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_RETURN:
                ...
                for i in range(len(level)):
                    ...
                    # CLEMENT : My addition lul
                        match level[i][j]:
                            case 1:
                                print(f'let singlePlat{singlePlatCount}_{active_level}  = new Platform(new CANNON.Vec3({j+(active_level*18)},{len(level)-i},0), player, 1)')
                                singlePlatCount += 1
                            case 2:
                                print(f'let deathBox{deathBoxCount}_{active_level}  = new DeathBox(new CANNON.Vec3({j+(active_level*18)},{len(level)-i},0), player)')
                                deathBoxCount += 1
                            case 3:
                                print(f'let fivePlat{fivePlatCount}_{active_level}  = new Platform(new CANNON.Vec3({j+(active_level*18)},{len(level)-i},0), player, 5)')
                                fivePlatCount += 1
                ...
                if buttons[i].collidepoint(event.pos):
                    ...
                    if i == 0:
                        ...
                        if active_phase > 0:
                            ...
                            active_phase = 3
            if not button_press:
                coords = pygame.mouse.get_pos()[0] // TILE_SIZE, pygame.mouse.get_pos()[1] // TILE_SIZE
                try:
